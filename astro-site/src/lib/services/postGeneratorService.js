...
import openaiService from './aiProviders/openaiService.js';
import geminiService from './aiProviders/geminiService.js';
import claudeService from './aiProviders/claudeService.js';
...
Next Section of Post (HTML):`;
      ...
      const cleanContentHtml = (html) => {
        return html
          ...
          .replace(/(<p>\s*<br\s*\/?>\s*<\/p>|<p>\s*<\/p>|<br\s*\/?>|\s)+$/gi, '') 
          .replace(/<p><strong>SEO Title:<\/strong>.*?<\/p>/gi, '') 
      ...
      const contentDoc = {
        ...
        generationTime,
        featured_image: options.featuredImage || null,
...
export default postGeneratorService;
